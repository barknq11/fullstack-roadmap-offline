...
// ===== Graph Rendering =====
let graphScale = 1, graphPanX = 0, graphPanY = 0, isPanning = false, panStartX = 0, panStartY = 0;
...
function renderGraph() {
  ...
  renderable.forEach(n => {
    ...
    svgContent += `<g data-nodeid="${esc(nodeId)}" ${isClickable?`style="cursor:pointer" onclick="selectTopicFromGraph('${esc(nodeId)}')"`:''}>`;
...
svgEl.addEventListener('mousedown', e => { isPanning=true; panStartX=e.clientX-graphPanX; panStartY=e.clientY-graphPanY; });
svgEl.addEventListener('mousemove', e => { if (!isPanning) return; graphPanX=e.clientX-panStartX; graphPanY=e.clientY-panStartY; updateGraphTransform(); });
